...
def ReadData():
    ...
    for data in wb["Ani"]:
        ...
        if d1 not in AniDict:
            AniDict[d1] = {}

        for idx_data, data in enumerate(datalist[1:6]):
            if data == None:
                AniDict[d1][f"Song{idx_data+1}"] = ""
            else:
                AniDict[d1][f"Song{idx_data+1}"] = str(data)

        ansidx = 6
        AniDict[d1]["Hint"] = makehint(datalist[ansidx])

        AniDict[d1]["Ans"] = [str(x) for x in datalist[ansidx:] if x]

        for ans in AniDict[d1]["Ans"]:
            if ans not in AnswerDict:
                for a in makeAnswer(ans):
                    if a not in AnswerDict:
                        AnswerDict[a] = []
                    AnswerDict[a].append(d1 + 2000)
    ...
    for data in wb["Song"]:
        ...
        if d1 not in MyDict:
            ...
            MyDict[d1]["CateGory"] = []
            MyDict[d1]["CateNum"] = datalist[3]

        if len(MyDict[d1]["CateGory"]) < d2 + 1:
            MyDict[d1]["CateGory"].append({})

        for idx_data, data in enumerate(datalist[4:9]):
            if data == None:
                MyDict[d1]["CateGory"][d2][f"Song{idx_data+1}"] = ""
            else:
                MyDict[d1]["CateGory"][d2][f"Song{idx_data+1}"] = str(data)

        MyDict[d1]["CateGory"][d2]["Address"] = str(datalist[9])
        MyDict[d1]["CateGory"][d2]["Time"] = [x if x else 0 for x in [datalist[10], datalist[11]]]

        ansidx = 12
        MyDict[d1]["CateGory"][d2]["Hint"] = makehint(datalist[ansidx])
        MyDict[d1]["CateGory"][d2]["Ans"] = [str(x) for x in datalist[ansidx:] if x]

        for ans in MyDict[d1]["CateGory"][d2]["Ans"]:
            for a in makeAnswer(ans):
                if a not in AnswerDict:
                    AnswerDict[a] = []
                AnswerDict[a].append(SCount[0] + 3000)
...
def MakeTxt():
    ...
    Ani1Txt = open(txtpath + "\\" + "제목 정보 1.txt", "w", encoding="utf-8")
    Ani2Txt = open(txtpath + "\\" + "제목 정보 2.txt", "w", encoding="utf-8")
    Ani3Txt = open(txtpath + "\\" + "제목 정보 3.txt", "w", encoding="utf-8")
    Ani4Txt = open(txtpath + "\\" + "제목 정보 4.txt", "w", encoding="utf-8")
    Ani5Txt = open(txtpath + "\\" + "제목 정보 5.txt", "w", encoding="utf-8")

    AniHint1Txt = open(txtpath + "\\" + "제목 초성 - 1 번째.txt", "w", encoding="utf-8")
    AniHint2Txt = open(txtpath + "\\" + "제목 초성 - 2 번째.txt", "w", encoding="utf-8")
    AniHint3Txt = open(txtpath + "\\" + "제목 초성 - 3 번째.txt", "w", encoding="utf-8")

    AniAnswerTxt = open(txtpath + "\\" + "제목 정답 시 (노래정보 1 & 2 합진 것).txt", "w", encoding="utf-8")
    ...
    SongHint1Txt = open(txtpath + "\\" + "노래 초성 - 1 번째.txt", "w", encoding="utf-8")
    SongHint2Txt = open(txtpath + "\\" + "노래 초성 - 2 번째.txt", "w", encoding="utf-8")
    SongHint3Txt = open(txtpath + "\\" + "노래 초성 - 3 번째.txt", "w", encoding="utf-8")

    SongAnswerTxt = open(txtpath + "\\" + "노래 정답 시 (노래정보 1 & 2 합진 것).txt", "w", encoding="utf-8")
    
    LengthTxt = open(txtpath + "\\" + "노래 길이.txt", "w", encoding="utf-8")

    CateArrowTxt = open(txtpath + "\\" + "CateArrowList.txt", "w", encoding="utf-8")
    CateLengthTxt = open(txtpath + "\\" + "CateLengthList.txt", "w", encoding="utf-8")

    keylist1 = sorted(AniDict.keys())
    for key in keylist1:
        ASDF = AniDict[key]
        ...
        AniHint1Txt.write(mytext)

        if "'" in vals[1]:
            mytext = 'Db("{}"), '.format(vals[1].replace('"', "\\\""))
        else:
            mytext = "Db('{}'), ".format(vals[1])
        AniHint2Txt.write(mytext)

        if "'" in vals[2]:
            mytext = 'Db("{}"), '.format(vals[2].replace('"', "\\\""))
        else:
            mytext = "Db('{}'), ".format(vals[2])
        AniHint3Txt.write(mytext)

    keylist2 = sorted(MyDict.keys())
    CateLengthTxt.write("0, ")
    cate_len = 0

    for key in keylist2:
        ASDF = MyDict[key]

        for asdf in ASDF["CateGory"]:
            val = asdf["Song1"]
            if asdf["Song2"] and asdf["Song1"] != asdf["Song2"]:
                val = f"{val}({asdf['Song2']})"
            ...
            if "'" in vals[0]:
                ...
                mytext = "Db('{}'), ".format(vals[0])
            SongHint1Txt.write(mytext)

            if "'" in vals[1]:
                mytext = 'Db("{}"), '.format(vals[1].replace('"', "\\\""))
            else:
                mytext = "Db('{}'), ".format(vals[1])
            SongHint2Txt.write(mytext)

            if "'" in vals[2]:
                mytext = 'Db("{}"), '.format(vals[2].replace('"', "\\\""))
            else:
                mytext = "Db('{}'), ".format(vals[2])
            SongHint3Txt.write(mytext)

            CateArrowTxt.write(f'{ASDF["CateNum"]}, ')

        cate_len += len(ASDF["CateGory"])
        CateLengthTxt.write(f"{cate_len}, ")
...
def Running():
    try:
        ...
        for elememtKey in sorted(MyDict.keys()):
            ani = MyDict[elememtKey]

            for song in ani["CateGory"]:
                idx+= 1
    ...
    for elememtKey in sorted(MyDict.keys()):
        ...
        for song in ani["CateGory"]:
            idx2+= 1
            in_dict = {"addr":song["Address"], "SongName":song["Song1"], "idx1":idx1, "idx2":idx2, "total":SCount[0], "cut1": song["Time"][0], "cut2": song["Time"][1], "SI":SI, "mono":isMono}
            CutFile(**in_dict)