...
def makeAnswer(word):
    ...
    Spaceword = [AnswerChanger(str(word)), AnswerChanger(str(word).replace(" ",""))]
...
def ReadData():
    ...
    for data in wb["Main"]:
        ...
        MyDict[d1]["Address"] = str(datalist[6])
        MyDict[d1]["Time"] = [x if x else 0 for x in [datalist[7], datalist[8]]]
    ...
    for data in wb["Lyrics"]:
        ...
        if d1 not in LyricsDict:
            LyricsDict[d1] = []

        LyricsDict[d1].append({})
        LyricsDict[d1][-1]["Timing"] = calculateTiming(timing)
        ls = [str(x) if x != None else " " for x in datalist[4:8]]

        for lidx, lval in enumerate(ls):
            if "\\x12" not in lval and "\\x13" not in lval:
                if "\\left" in lval:
                    ls[lidx] = lval.replace("\\left", "")
                else:
                    ls[lidx] = f"\\x13{lval}"

            LyricsDict[d1][-1][f"L{lidx+1}"] = ls[lidx]
...
def MakeTxt():
    ...
    TimingTxt = open(txtpath + "\\" + "가사 - Timing.txt", "w", encoding="utf-8")
    TimingLengthTxt = open(txtpath + "\\" + "가사 - Timing Length.txt", "w", encoding="utf-8")

    L1Txt = open(txtpath + "\\" + "가사 - L1.txt", "w", encoding="utf-8")
    L2Txt = open(txtpath + "\\" + "가사 - L2.txt", "w", encoding="utf-8")
    L3Txt = open(txtpath + "\\" + "가사 - L3.txt", "w", encoding="utf-8")
    L4Txt = open(txtpath + "\\" + "가사 - L4.txt", "w", encoding="utf-8")

    L1Txt.write("const Lyrics1 = [\n")
    L2Txt.write("const Lyrics2 = [\n")
    L3Txt.write("const Lyrics3 = [\n")
    L4Txt.write("const Lyrics4 = [\n")
    ...
    for key in keylist:
        ...
        TimingLengthTxt.write(f"{len(QWER)}, ")
        ...
        TimingTxt.write("99999999, 99999999,],\n")
        L1Txt.write("],\n")
        L2Txt.write("],\n")
        L3Txt.write("],\n")
        L4Txt.write("],\n")
