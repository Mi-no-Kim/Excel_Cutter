...
class MyDict:
    ...
    def append(self, appendType ,datalist, numberCut=None):
        ...
        if appendType == "Song":
            ...
            self.cateDict[d2].songs.append(d1)

            self.songDict[d1].idx2 = d2
            self.songDict[d1].basic_info = [str(data) if data else "" for data in datalist[3:8]]
            self.songDict[d1].address = str(datalist[8])
            self.songDict[d1].time = [x if x else 0 for x in [datalist[9], datalist[10]]]
        ...
        if appendType == "Cate":
            ...
            if d1 not in self.cateDict:
                self.cateDict[d1] = MyCate()
                self.cateDict[d1].idx1 = d1
                self.cateDict[d1].name = datalist[1]
                self.cateDict[d1].songs = []
            else:
                raise IndexError

            self.cateDict[d1].cate_name = str(datalist[1])
...
class MyType:
    ...
    def ReadData(self):
        ...
        for data in self.wb["Category"]:
            ...
            self.MyDict.append("Cate", datalist=datalist, numberCut=[2])
    ...
    def MakeTxt(self):
        ...
        for key in self.keylist:
            ASDF = self.MyDict.cateDict[key]

            val = len(ASDF.songs)
            self.TxtDict.add_text("CateLengthTxt", "{}, ".format(val))

            val = ASDF.name
            self.TxtDict.add_text("CateNameTxt", "Db('{}'), ".format(val))
